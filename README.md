...
**Models**
- Six Whisper variants, 99 languages each: `tiny` (77 MB, ~20× realtime), `base` (147 MB, ~12×),
  `small` (487 MB, ~8×), `medium` (1.5 GB, ~4×), `large-v3 turbo` (3.2 GB, ~6×) and `large-v3`
  (3.1 GB, ~3×). Download and delete them from inside the app.
...
The first time you start a session, Unlimeety downloads the selected Whisper model into `~/Library/Application Support/Unlimeety/models/whisperkit/`. The **Live** tab defaults to `base` (147 MB); everything that transcribes after the fact — the **Record** tab's automatic run and the transcribe-settings screen — uses `large-v3` (3.1 GB), since it is not racing speech and can afford the accuracy. This happens once per model; later sessions reuse the cache. A progress indicator is shown while it downloads — be on Wi-Fi for the first run.
...
- **Summarization talks to the cloud by default.** The default provider, Claude Code,
  ...
  **Ollama is the only fully offline provider**; pick it if the summary must stay
  on-device.
