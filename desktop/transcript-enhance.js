...
/// The pieces of a participant entry's local part: `i.grinko@corp.com` →
/// `['i', 'grinko']`. An entry that is not an address has no segments and can
...
///
/// `initials` lets a one-letter segment — how `i.grinko@` spells a given name —
...
function bindsEveryPart(parts, entry, initials = false) {
    ...
    return parts.every((part) => {
        ...
        // letter, and it would otherwise bind to the initial in `i.grinko@`.
...
function attestation(name, body, participants) {
    ...
    // onto another colleague's surname — "Ольга" spoken and `i.grinko@` listed
    // is not Ольга Гринько.
