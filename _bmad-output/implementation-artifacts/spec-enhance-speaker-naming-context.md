...
|----------|--------------|---------------------------|----------------|
| Surname from email | `Beta -> Илона Гринько`; "Илон" spoken, `i.grinko@` listed | Accepted; marker becomes `Илона Гринько (Beta)` | N/A |
| Nothing spoken | `Beta -> Ирина Гринько`; `i.grinko@` listed, no part spoken | Rejected — binding never attests alone | Placeholder kept |
| Transliteration drift | `Delta -> Сергей Куприянов`; "Сергей" spoken, `s.kupriianov@` (`ya`/`ia`, distance 1) | Accepted within the fuzzy budget | N/A |
| Ambiguous binding | `Beta -> Илона Гринько`; both `i.grinko@` and `a.grinko@` listed | Rejected — more than one participant binds | Placeholder kept |
| Wrong surname pairing | `Gamma -> Ольга Гринько`; "Оля" spoken, `o.volkova@` and `i.grinko@` listed | Rejected — no single email matches the unspoken part | Placeholder kept |
| Sub-two-char part | `Beta -> Х Гринько`; "Гринько" binds | Rejected — a one-character part binds to nothing | Placeholder kept |
...
- `desktop/transcript-enhance.js` `renameParticipantsLine` -- dedupes by string, so `i.grinko@…, Beta` becomes `i.grinko@…, Илона Гринько (Beta)` — one person twice. Binding knows which entry matched.
...
- Read-only evidence: `~/Downloads/Meet_Transcripts/Team onboarding 15-04 01-09-26.txt` (body 41530 chars, 165 turns) and its summary in `2 - Areas/Unlimit/meetings/daily/` (8 placeholders resolved, citing the emails).
...
**Acceptance Criteria:**
- Given the failing transcript's body and participant list, when the naming pass runs with a reply naming `Beta -> Илона Гринько` and `Gamma -> Ольга Волкова`, then both are accepted (their given names are spoken) and their markers read `Илона Гринько (Beta)` and `Ольга Волкова (Gamma)`.
...
- Given a transcript already carrying `Илона Гринько (Beta)`, when Enhance runs again, then that speaker is no longer a placeholder and is left alone.
- Given `node --test` in `desktop/`, when the suite runs, then every existing gate still passes.

## Spec Change Log

- **Finding:** All three review layers independently verified that email binding attested a name with nothing spoken — on a body naming no one, `Beta -> Ирина Гринько` and `Beta -> Игорь Гринько` both bound to `i.grinko@` and were written, and `Мария Смирнов` bound to `m.smirnova@` at one edit. The model was choosing a given name from a single initial.
  ...
  **KEEP:** The transliteration table and its calibration (14 of 15 surnames at distance 0, `Куприянов` at 1) were right and must survive. So must binding a name to a *single* entry rather than to the list, per-turn middle-trimming over a stride, water-filling so short turns survive whole, and reusing `glossary.withinDistance` instead of a second Levenshtein.

## Design Notes

The transliteration table is calibrated against one real meeting's 16 participant addresses: 14 surnames land at distance 0, `Куприянов`→`kupriyanov` vs `kupriianov` at 1. Exact equality is therefore wrong, and a fuzzy budget of ~2 for segments of 5+ chars (`glossary.fuzzyBudget`'s shape) is right. Do not widen it: at two edits `Смирнов`/`Смирнова` are already one person's two spellings, and a third reaches a different colleague.

```
"Илон" spoken + Илона Гринько vs i.grinko@              -> given name heard, grinko binds  ACCEPT
nothing spoken + Ирина Гринько  vs i.grinko@              -> binding alone attests nothing   REJECT
"Оля" spoken   + Ольга Гринько  vs o.volkova@, i.grinko@  -> unspoken part binds neither     REJECT
```

**Accepted limitation.** A given name and a surname drawn from the *same* address can still be the wrong pairing, because `initial.surname` cannot distinguish two given names sharing an initial. Observed live: the model answered `Валерий Орлов` for a speaker the transcript really does address as "Валерий", taking the surname from `v.orlov@example.com` — which belongs to Виктор Орлов, while Валерий is the recorder, whose own address is not on the participant list at all. The summarize pass makes the identical error on the same file, so it is a model-level pairing judgement, not something either pass can catch mechanically. The user reviewed this and accepted the risk rather than narrowing output to spoken-only surnames. Do not add heuristics for it; what code *can* enforce — one part spoken, every part fitting one single entry, no entry claimed twice — is enforced.

Per-turn trimming beats the stride at equal budget: at 40000 the stride kept 18922 chars and lost "Илон"; trimming keeps 26717 and every naming cue but one. At 120000 the body passes untouched.
...
- Re-run Enhance on a copy of `Team onboarding 15-04 01-09-26.txt` with its placeholders restored, and confirm the markers and the `Participants:` line carry full names in `Name (Label)` form, with no address left beside the name bound to it.
